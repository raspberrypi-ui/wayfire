   ...
   wf::texture_t::texture_t(wlr_texture *texture)
     {
        ...
        if (!runtime_config.use_pixman)
          ...
          {
             /* XXX: TODO: Implement tex_id for Pixman */
             assert(wlr_texture_is_pixman(texture));
             wlr_pixman_texture_attribs attribs;
             wlr_pixman_texture_get_attribs(texture, &attribs);
             this->target = attribs.target;
             /* this->image = attribs.image; */
             has_alpha = attribs.has_alpha;
          }
